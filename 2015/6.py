import numpy as np
input: str = """toggle 461,550 through 564,900
...
instructions = [(ins.split()[1], list(map(int, ins.split()[2].split(','))), list(map(int, ins.split()[-1].split(',')))) if len(ins.split()) == 5
                else (ins.split()[0], list(map(int, ins.split()[1].split(','))), list(map(int, ins.split()[-1].split(',')))) for ins in input.split('\n')]


lights_p1 = np.zeros(shape=(1000, 1000), dtype=np.int8)
lights_p2 = np.zeros(shape=(1000, 1000), dtype=np.int8)

for op, [a, b], [c, d] in instructions:
    for i in range(a, c+1):
        for j in range(b, d+1):
            ...
            if op == "off":
                lights_p1[i, j] = 0
                lights_p2[i, j] -= 1 if lights_p2[i, j] > 0 else 0
            if op == "toggle":
                lights_p1[i, j] ^= 1
                lights_p2[i, j] += 2

print(f"Part 1: {lights_p1.sum()}")
print(f"Part 2: {lights_p2.sum()}")
